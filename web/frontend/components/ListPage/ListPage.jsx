...
const ListPage = ({ title, resourceName, fetchUrl, deleteUrl, createUrl, editUrl, itemsPerPage = 50 }) => {
    ...
    const statusToBadgeProps = (status) => {
        switch (status.toLowerCase()) {
            case 'pending':
                return { status: 'warning', children: 'Pending' };
            case 'processed':
                return { status: 'success', children: 'Processed' };
            case 'sent':
                return { status: 'info', children: 'Sent' };
            case 'archive':
                return { status: 'attention', children: 'Archived' };
            default:
                return { status: 'default', children: status };
        }
    };
    useEffect(() => {
